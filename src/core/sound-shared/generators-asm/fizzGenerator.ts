/**
 * Fizz Sound Generator - Assembly Implementation
 *
 * Direct port of do_fizz_sound() from Sound.c:299-326
 * Uses 68K assembly emulator to exactly match original behavior
 *
 * Creates fizzing/hissing sound effect with decreasing amplitude
 *
...
export const createFizzGenerator = (): SampleGenerator => {
  ...
  // State variables
  let fizzcount = FIZZ_COUNT_START
  let isActive = false
  ...
  const generateChunk = (): Uint8Array => {
    ...
    // Implementation of do_fizz_sound
    fizzcount--
    if (fizzcount > 0) {
      // amp = fizzcount + 40
      const amp = fizzcount + 40
  ...
  const hasEnded = (): boolean => {
    // Fizz sound has ended when it's no longer active
    return !isActive
  }
