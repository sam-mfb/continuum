/**
 * Bunker Sound Generator - Assembly Implementation
 *
 * Direct port of do_bunk_sound() from Sound.c:208-234
 * Uses 68K assembly emulator to exactly match original behavior
 *
 * Creates descending tone for bunker destruction
 *
...
const SNDBUFLEN = 370
const BUNK_AMP = 104 // Initial amplitude from Sound.c:478
const BUNK_AMPCHANGE = 3 // Amplitude change rate from Sound.c:479

export const createBunkerGenerator = (): SampleGenerator => {
  // Create 68K emulator context
  const asm = build68kArch()

  // State variables (simulating A5-relative storage)
  let amp = BUNK_AMP
  let ampchange = BUNK_AMPCHANGE
  let period = 2 // Starting period from Sound.c:477
  let isActive = false
  ...
  const reset = (): void => {
    amp = BUNK_AMP
    ampchange = BUNK_AMPCHANGE
    period = 2 // Starting period from Sound.c:477
    isActive = true
    autoStart = false
  }
  ...
  const hasEnded = (): boolean => {
    // Bunker sound has ended when it's no longer active
    return !isActive
  }
