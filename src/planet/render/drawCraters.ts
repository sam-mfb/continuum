...
import { CRATERHT } from '@/figs/types'
...
export function drawCraters(deps: {
  ...
  return screen => {
    const { craters, numcraters, scrnx, scrny, worldwidth, on_right_side, craterImages } = deps
      ...
      if (crat.y >= top && crat.y <= bot) {
        ...
        if (crat.x >= left && crat.x < right) {
          ...
          const craterImage: MonochromeBitmap = {
            width: 32,
            height: CRATERHT,
            data: rot === 0 ? craterImages.background1 : craterImages.background2,
