...
import { nepatch, enepatch, epatch, sepatch, ssepatch, npatch } from './patchPatterns'
...
export function oneClose(
  ...
  for (const [n, m] of closeEndpoints) {
    ...
    const addWhite = (x: number, y: number, ht: number, data: number[]) => {
      patches.push({
        id: `p${whiteIdCounter++}`,
        x,
        y,
    ...
    switch (dir1) {
      ...
      case 11:
        if (dir1 === 11) { // Only execute if we didn't fall through from case 10
          if (dir2 === 9) {
            i = 2
          } else if (dir2 === 10) {
            i = 4
