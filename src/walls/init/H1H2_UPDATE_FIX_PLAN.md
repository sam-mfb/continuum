...
### 2.2 Move updateWallOptimization tests to processCloseWalls tests

- Take existing test cases for `updateWallOptimization`
- Adapt them to test the same functionality within `processCloseWalls`
- For example:
  - "applies h1/h2 updates to correct walls by ID" → test that `finalWalls` has correct updates
  - "preserves walls without updates" → test that unchanged walls remain unchanged in `finalWalls`
  - "handles multiple updates to same wall" → test aggregation behavior
  - "creates new wall objects (immutability)" → test that `finalWalls` are new objects
- These tests verify the update logic still works correctly after being moved
