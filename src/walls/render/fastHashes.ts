...
export const fastHashes = (
  ...
  const top = data.viewport.y - SCREEN_TOP_MARGIN
  const bot = data.viewport.b
  let left = data.viewport.x - SCREEN_LEFT_MARGIN
  ...
  for (let i = 0; i < 2; i++) {
    ...
    while (jIndex < junctions.length && junctions[jIndex]!.x <= right) {
      ...
      const drawY = junction.y - data.viewport.y
      ...
      if (
        ...
        orPatternToScreen(newScreen, address + newScreen.rowBytes * 2, shifted2)

        // Line 3: shift by 2 more (line 896-897)
        const shifted3 = shifted2 >>> 2
        orPatternToScreen(newScreen, address + newScreen.rowBytes * 3, shifted3)

        // Line 4: shift by 2 more (line 898-899)
        const shifted4 = shifted3 >>> 2
        orPatternToScreen(newScreen, address + newScreen.rowBytes * 4, shifted4)

        // Line 5: special handling with AND operation (lines 900-904)
        const temp = shifted4 >>> 2
        const shifted5 = (shifted4 >>> 1) & temp
        orPatternToScreen(newScreen, address + newScreen.rowBytes * 5, shifted5)
      } else {
        // Use the general draw_hash function for edge cases (lines 871-874)
        drawHash(newScreen, drawX, drawY)
      }
