...
async function loadStatusBar(): Promise<Uint8Array> {
  try {
    ...
    if (decoded.length !== SBARSIZE) {
      console.warn(
        `Status bar size mismatch: expected ${SBARSIZE}, got ${decoded.length}`
      )
    }
...
export const createStatusBarDemo =
  ...
  (bitmap, frame, _env) => {
      ...
      loadStatusBar().then(
        template => {
          console.log('Status bar loaded:', {
            ...
            first10Bytes: Array.from(template.slice(0, 10))
    ...
    for (
      let i = 0;
      i < Math.min(state.statusBarTemplate.length, SBARSIZE);
      i++
    ) {
