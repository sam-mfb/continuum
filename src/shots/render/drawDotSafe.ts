...
/**
 * Batch draw multiple dots efficiently
 * This is an optimization for drawing many bunker shots at once
 * 
 * @param dots - Array of {x, y} coordinates
 * @param screen - The bitmap to draw on
 * @returns Modified bitmap with all dots drawn
 */
export function drawDotsSafe(
  dots: Array<{x: number, y: number}>,
  screen: MonochromeBitmap
): MonochromeBitmap {
  for (const dot of dots) {
    drawDotSafe(dot.x, dot.y, screen)
  }
  return screen
}