...
export function drawShipShot(deps: {
  ...
  return screen => {
    ...
    // Get random value to determine filled vs empty
    // moveq #3, D0
    // and.b (data), D0
    const randValue = EXPL_RANDS[randCounter & 0x7F]! & 3

    if (randValue === 0) {
      // Draw filled shot (@full)
      for (let i = 0; i < 4; i++) {
        // moveq #0, D0
        // move.w (A1)+, D0
        let data = FILLED_SHOT[i]!
        
        // lsl.l x, D0
        data = data << leftRot
        
        // or.l D0, (A0)
        asm.instructions.or_l(newScreen.data, asm.A0, data)
      ...
      for (let i = 0; i < 4; i++) {
        ...
        if (asm.A0 < newScreen.data.length - 3) {
          screenData = (newScreen.data[asm.A0]! << 24) |
