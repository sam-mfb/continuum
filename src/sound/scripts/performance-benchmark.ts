...
function getRealtimeFactor(samplesPerSecond: number): string {
  ...
  const color =
    factor >= 10 ? colors.green : factor >= 5 ? colors.yellow : colors.red
...
async function runBenchmark() {
  ...
  console.log(
    `Target: Generate audio at least 10x faster than real-time (${SAMPLE_RATE} Hz)`
  )
...

  {
    ...
    printResult(
      'Time per 370-byte chunk:',
      formatTime((CHUNK_SIZE / samplesPerSecond) * 1000)
    )
  }
...

  {
    ...
    for (const size of bufferSizes) {
      ...
      console.log(
        `${size.toString().padStart(4)} samples: ` +
...

  {
    ...
    for (const { name, generator } of generatorTests) {
      ...
      console.log(
        `${name.padEnd(15)} ${formatTime(timePerChunk)}/chunk, ` +
          `${getRealtimeFactor(samplesPerSecond)}`
      )
    }
  }
...

  {
    ...
    for (const [name, generator] of Object.entries(generators)) {
      ...
      console.log(
        `${name.padEnd(15)} ${formatTime(avgTime)}/chunk ` +
...

  {
    ...
    printResult('Real-time factor:', getRealtimeFactor(samplesPerSecond))
    printResult(
      'Avg time per 512-sample request:',
      formatTime(avgTimePerIteration)
    )
  }
...

  {
    ...
    for (const callbackSize of callbackSizes) {
      ...
      console.log(
        `  Min/Max time:  ${formatTime(minTime)} / ${formatTime(maxTime)}`
      )
      console.log(
        `  Safety margin: ${safetyMargin >= 10 ? colors.green : safetyMargin >= 5 ? colors.yellow : colors.red}${safetyMargin.toFixed(1)}x${colors.reset}`
      )
    }
  }
...

  {
    ...
    printResult(
      'Through buffer manager:',
      formatTime(bufferPerChunk) + '/chunk'
    )
    printResult(
      'Overhead:',
      formatTime(overhead) + ` (${overheadPercent.toFixed(1)}%)`
    )
  }
  ...
  console.log(
    `${colors.bright}✓ All measurements show performance well above real-time requirements${colors.reset}`
  )
  console.log(
    `${colors.bright}✓ Target of < 3ms per 370-byte chunk is easily met${colors.reset}`
  )
}
